...
**Data Flow for the Screenshot Shown**
The screenshot shows "tamil albums" search → clicked album "Raga of Revenge" → 1 song playing:

User typed "tamil albums" → searchSaavnAlbums() ran 10+ JioSaavn queries → returned album grid
User clicked "Raga of Revenge" → getSaavnAlbumSongs({ albumId }) fetched songs + YouTube ID
Track loaded → searchYouTube("Raga of Revenge ...") found video → YouTube IFrame plays full song
NowPlayingBar shows at bottom with progress 1:13 / 2:11
MusicVisualizer renders equalizer bars, floating symbols, and cracker lights in background
